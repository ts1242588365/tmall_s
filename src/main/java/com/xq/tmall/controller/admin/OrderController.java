...
public class OrderController extends BaseController {
	...
	public String goToPage(HttpSession session, Map<String, Object> map) {
		...
		List<ProductOrder> productOrderList = productOrderService.getList(null, null,
				new OrderUtil("productOrder_id", true), pageUtil);
			...
			for (ProductOrderItem productOrderItem : productOrderItemList) {
				...
				if (product != null) {
					...
					product.setSingleProductImageList(
							productImageService.getList(productId, (byte) 0, new PageUtil(0, 1)));
				}
	...
	public String updateOrder(@PathVariable("order_id") String order_id) {
		...
		logger.info("整合订单信息");
		ProductOrder productOrder = new ProductOrder().setProductOrder_id(Integer.valueOf(order_id))
				.setProductOrder_status((byte) 2).setProductOrder_delivery_date(new Date());
	...
	public String getOrderBySearch(@RequestParam(required = false) String productOrder_code/* 订单号 */,
		...
		// 移除不必要条件
		if (productOrder_status_array != null
				&& (productOrder_status_array.length <= 0 || productOrder_status_array.length >= 5)) {
		...
		// 封装查询条件
		ProductOrder productOrder = new ProductOrder().setProductOrder_code(productOrder_code)
		...
		List<ProductOrder> productOrderList = productOrderService.getList(productOrder, productOrder_status_array,
				orderUtil, pageUtil);
